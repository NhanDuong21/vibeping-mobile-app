    ...
    await new Promise<void>((resolve, reject) => {
      const writing = database
        ...
        .put({
          ...
          currentWork: null,
          usageLimits: { state: 'stale', readAt: null, cursor: '1', windows: [] },
          unreadCount: 1,
          events: [event],
          nextCursor: null,
          pendingReadIds: [],
...
test('cached activity survives laptop stop and browser offline', async ({ context, page }) => {
...
test('IndexedDB version one upgrades to the current activity cache schema', async ({ page }) => {
  await page.goto('/manifest.webmanifest');
  await page.evaluate(async () => {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      opening.onupgradeneeded = () => opening.result.createObjectStore('legacy-activity');
      opening.onsuccess = () => { opening.result.close(); resolve(); };
      opening.onerror = () => reject(opening.error);
    });
  });
  await page.goto('/activity?cache=upgrade');
  await expect(page.getByRole('heading', { name: 'Bạn có thể rời laptop' })).toBeVisible();
  const schema = await page.evaluate(async () => {
    ...
    return { version: database.version, stores: [...database.objectStoreNames] };
  });
  expect(schema).toEqual({ version: 2, stores: ['activity-feed', 'legacy-activity'] });
});
