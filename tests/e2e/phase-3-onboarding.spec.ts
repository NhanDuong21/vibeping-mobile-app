...
async function mockStart(page: Page, status: Record<string, unknown>): Promise<void> {
...
test('regular Safari tab gives install instructions and never asks permission', async ({
  ...
  await expect(page.getByText('VibePing sẽ không hỏi quyền thông báo')).toBeVisible();
});
...
test('installed app waits for an explicit tap before requesting default permission', async ({
  ...
  await page.getByRole('button', { name: 'Tiếp tục' }).click();
  await expect(page.getByRole('heading', { name: 'Bật thông báo' })).toBeVisible();
