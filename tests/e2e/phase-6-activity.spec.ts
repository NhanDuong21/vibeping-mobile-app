...
const completed = {
  ...
  projectName: 'du-an-co-ten-rat-dai-de-kiem-tra-kha-nang-xuong-dong-tren-dien-thoai',
  ...
  await page.route('**/api/v1/bootstrap', (route) =>
    route.fulfill({
      ...
      body: JSON.stringify({
        ...
        unreadCount,
        usageLimits: {
          state: 'available', readAt: '2026-09-02T00:02:00Z', cursor: '1',
          windows: [{
...
test('mixed unread activity paginates and opens an exact detail deep link', async ({ page }) => {
  ...
  await page.route(/\/api\/v1\/events(\?.*)?$/, (route) => {
    ...
    return route.fulfill({
      ...
      body: JSON.stringify(
        cursor
          ? { events: [{ ...completed, id: 'event-old', isRead: true }], nextCursor: null, unreadCount: 2 }
          : { events: [permission, completed], nextCursor: 'event-completed', unreadCount: 2 },
      ),
    });
  });
  await page.route('**/api/v1/events/event-permission', (route) =>
    route.fulfill({ contentType: 'application/json', body: JSON.stringify(permission) }),
  );
  ...
  await page.route('**/api/v1/events/event-permission/read', (route) =>
    route.fulfill({ contentType: 'application/json', body: JSON.stringify({ state: 'read', unreadCount: 1 }) }),
  );
  ...
  const olderPage = page.waitForResponse((response) =>
    response.url().includes('/api/v1/events?') && response.url().includes('cursor='),
  );
  ...
  await expect(page.getByRole('heading', { name: 'Codex cần bạn xác nhận' })).toBeVisible();
  await expect(page.getByText('Đã đọc')).toBeVisible();
});

test('SSE reconnect and duplicate delivery add one activity item', async ({ page }) => {
  await page.route('**/sw.js', (route) => route.abort());
...
test('a missing notification deep link has a calm recovery path', async ({ page }) => {
  await page.goto('/activity/events/missing-event');
  await expect(page.getByRole('heading', { name: 'Hoạt động này không còn trên laptop' })).toBeVisible();
