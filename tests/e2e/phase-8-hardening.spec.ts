...
test("a corrupted optional activity cache is discarded and rebuilt from the laptop", async ({
  page,
}) => {
    ...
    .poll(() =>
      page.evaluate(async () => {
        ...
        return Array.isArray(
          (snapshot as { events?: unknown[] } | undefined)?.events,
        );
      }),
    )
    .toBe(true);
...
test("server-provided activity text is rendered literally without executable markup", async ({
  page,
}) => {
  ...
  expect(
    await page.evaluate(() => Reflect.get(window, "__vibepingXss")),
  ).toBeUndefined();
});
