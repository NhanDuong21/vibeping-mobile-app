...
$packageName = 'VibePing-Windows-x64-v1.0.0-rc.3'
...
function Wait-Health {
    ...
    while ([DateTime]::UtcNow -lt $deadline) {
        try {
            ...
            if ($health.status -eq 'ok' -and $health.version -eq '1.0.0-rc.3') { return $health }
        }
...
try {
    ...
    if ($version -ne 'vibeping 1.0.0-rc.3') { throw "Sai phiên bản package: $version" }
