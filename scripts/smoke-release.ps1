...
$packageName = 'VibePing-Windows-x64-v1.2.0'
...
function Wait-Health {
    ...
    while ([DateTime]::UtcNow -lt $deadline) {
        try {
            ...
            if ($health.status -eq 'ok' -and $health.version -eq '1.2.0') { return $health }
        }
...
try {
    ...
    if ($version -ne 'vibeping 1.2.0') { throw "Sai phiên bản package: $version" }
